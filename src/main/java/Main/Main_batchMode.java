package Main;
import java.io.BufferedWriter;
import java.io.File;
import java.io.FileWriter;
import java.io.IOException;
import java.io.PrintWriter;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.Map;

import org.apache.commons.lang3.SerializationUtils;

import Controler.ControlerPlantVSZombie;
import Game.EnumActionJeu;
import Game.PlantVSZombieGame;
import Model.InputMap;
import ReseauNeurone.TrainExample;
import Strategie.StrategieReseauNeurone;
import Strategie.Strategy;
import Strategie.StrategyRandom;
import Utils.ParametreJeux;
import Utils.Position;




public class Main_batchMode {
	private static final Boolean VISUAL=false;
	private static  Boolean TEST=false;
	private static PrintWriter PR_OUT;
	private static FileWriter myWriter;				
	private static BufferedWriter bw;

	public static final String fichier_Log="./log/score/log.txt";

	private static int  nbSimulations = 1000;
	private static int  nbTurns = 200;
	private static int nbStepMax = 999999999;

	
	
	public static void main(String[] args) throws Exception {

		File file=new File(fichier_Log);
		if(file.exists())
		{
			file.delete();
		}

		try{
			myWriter = new FileWriter(fichier_Log,true);				
			 bw = new BufferedWriter(myWriter);
			 PR_OUT = new PrintWriter(bw);
			String csv="type;value";

			for(EnumActionJeu action:EnumActionJeu.values())
			{
				csv+=";"+action.toString();
			}

			 PR_OUT.println(csv);
			 PR_OUT.flush();
			  } catch (IOException e) {
				System.out.println("An error occurred.");
				e.printStackTrace();
			  }
        
        double gamma = ParametreJeux.gamma;
        double epsilon = ParametreJeux.epsilon;
        double alpha = ParametreJeux.alpha;
        
        int nEpochs = ParametreJeux.nEpochs;
        int batchSize = ParametreJeux.batchSize;


		String Layout="./src/main/java/Layout/layoutbase.txt";

		InputMap inputDeBase= new InputMap(Layout);
		
		int nombreInput=(int)(inputDeBase.size_x*inputDeBase.size_y*StrategieReseauNeurone.nombre_type_encodage)+1;
		
		
		StrategieReseauNeurone strat  = new StrategieReseauNeurone(epsilon, alpha, gamma, nEpochs, batchSize,nombreInput,EnumActionJeu.values().length);
		//strat.load("log/Reseau_interressant/neuralNetwork_23", true);
		
		//Strategy strat = new StrategyRandom();
		
		Strategy stratRandom=new StrategyRandom();
		
		
		
		for(int T = 0; T < nbStepMax; T++) {
			System.out.println("Mode Train");
			TEST=false;
			strat.modeTrain();
			ArrayList<TrainExample> trainExamples = play(inputDeBase, nbSimulations, nbTurns, strat, TEST,ParametreJeux.TRAIN_STRING);
			
			System.out.println("Learning");
			strat.learn(trainExamples); 
			System.out.println("Mode Test");
			TEST=true;
			strat.modeTest();
            play(inputDeBase, nbSimulations, nbTurns, strat, TEST,ParametreJeux.TEST_STRING);

			//RAND
			System.out.println("Mode Rand");
			play(inputDeBase, nbSimulations, nbTurns, stratRandom, false,ParametreJeux.RANDOM_STRING);


		}
		
	}
	
	
	public static ArrayList<TrainExample> play( InputMap map, int nbSimulations, int nbTurns, Strategy strat, boolean isTest,String csvPin) {
		

		ArrayList<PlantVSZombieGame> listPlantVSZombieGames = new ArrayList<PlantVSZombieGame>();
		
		for(int i = 0; i < nbSimulations; i++ ) {
			InputMap map_Jeu=SerializationUtils.clone(map);
			PlantVSZombieGame PVZGame = new PlantVSZombieGame(nbTurns,map_Jeu,strat);	
			PVZGame.initializeGame();
			if(i==0 && VISUAL /*&& isTest*/)
			{
				vizualize(PVZGame);
			}
			listPlantVSZombieGames.add(PVZGame);
		}
		

		System.out.println("Launch " + nbSimulations + " PLantVSZombie game");
		
		
		for(int i = 0; i < nbSimulations; i++ ) {
			listPlantVSZombieGames.get(i).launch();
		}
		
		
		for(int i = 0; i < nbSimulations; i++ ) {
		
			try {
				listPlantVSZombieGames.get(i).join();
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		}
		System.out.println("fini les join");
		if(VISUAL/*&& isTest*/)
		{
			listPlantVSZombieGames.get(0).updateObservateurJeu();
		}
		
		double avgScore = 0;

		Map<EnumActionJeu,Integer> actions =new HashMap<>();
		
		for(int i = 0; i < nbSimulations; i++ ) {
			avgScore += listPlantVSZombieGames.get(i).score;
			actions=fusionMap(actions,listPlantVSZombieGames.get(i).getAction());
		}
		
		avgScore = avgScore/nbSimulations;
		String csv=csvPin+";" + avgScore;
		System.out.println("average Score "+ avgScore);
		System.out.println();

		for(EnumActionJeu action:EnumActionJeu.values())
		{
			csv+=";"+actions.getOrDefault(action, 0);
		}

		PR_OUT.println(csv);
		PR_OUT.flush();

		
		
		ArrayList<TrainExample> trainExamples = new ArrayList<TrainExample>();

		for(int i = 0; i < nbSimulations; i++ ) {
			
			ArrayList<TrainExample> trainExamplesGame =  listPlantVSZombieGames.get(i).getTrainExample();

			for(int j = 0; j < trainExamplesGame.size(); j++) {
				
				trainExamples.add(trainExamplesGame.get(j));
				
			}
			
		}
		
		System.out.println("total number examples : " + trainExamples.size());
		
		
		return trainExamples;
	}

	public static void vizualize(int nbTurns, InputMap map,Strategy strat)
	{
		PlantVSZombieGame PVZGame = new PlantVSZombieGame(nbTurns,map,strat);	
		PVZGame.initializeGame();
		PVZGame.setTime(150);
		try {
			new ControlerPlantVSZombie(PVZGame,false);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}

	public static void vizualize(PlantVSZombieGame PVZGame)
	{
		PVZGame.initializeGame();
		PVZGame.setTime(300);
		try {
			new ControlerPlantVSZombie(PVZGame,false);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}

	public static Map<EnumActionJeu,Integer> fusionMap(Map<EnumActionJeu,Integer> l1,Map<EnumActionJeu,Integer>l2)
	{
		Map<EnumActionJeu,Integer> retour =new HashMap<>();

		int aux;

		for(EnumActionJeu action:EnumActionJeu.values())
		{
			aux=l1.getOrDefault(action, 0);
			aux+=l2.getOrDefault(action, 0);
			retour.put(action,aux);
		}

		return retour;
	}

	
}
